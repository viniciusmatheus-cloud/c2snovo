...
function handleFile(file){
  ...
  reader.onload=evt=>{
    try{
      ...
      const iN=idx(['name','nome']),iE=idx(['email']),iP=idx(['phone1','phone','telefone','celular']),iT=idx(['team','equipe','time']),iM=idx(['is_master','master','ismaster']);
      rows=dataRows.map(r=>({
        name:  iN>=0?String(r[iN]||'').trim():'',
        email: iE>=0?String(r[iE]||'').trim():'',
        phone1:iP>=0?String(r[iP]||'').trim():'',
        team:  iT>=0?String(r[iT]||'').trim():'',
        is_master:iM>=0?(r[iM]===true||r[iM]===1||String(r[iM]||'').toLowerCase()==='true'||String(r[iM]||'').toLowerCase()==='sim'):false,
      })).map(r=>{
...
function buildEditor(){
  ...
  document.getElementById('gh').innerHTML='<tr><th>Nome</th><th>E-mail</th><th>Telefone</th><th>Equipe</th><th style="width:70px;text-align:center">Master</th><th style="width:32px"></th></tr>';
  renderBody();
}
...
function renderBody(){
  ...
  rows.forEach((r,i)=>{
    ...
    tr.innerHTML=
      ...
      '<td>'+teamSel(i)+'</td>'+
      '<td style="text-align:center"><input type="checkbox" '+(r.is_master?'checked':'')+' title="Marcar como master" onchange="rows['+i+'].is_master=this.checked" style="width:16px;height:16px;cursor:pointer;accent-color:var(--blue)"/></td>'+
...
function addRow(){
  rows.push({name:'',email:'',phone1:'',team:'',is_master:false,_cid:subCompanies[0]?.company_id||'',_cname:subCompanies[0]?.company_name||'',_matched:false});
  renderBody();
...
async function startImport(){
  ...
  for(let i=0;i<valid.length;i++){
    ...
    for(let att=1;att<=3;att++){
      ...
      const body={name:u.name,email:u.email,username:uname,company_id:cid,is_master:!!u.is_master};
